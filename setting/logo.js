...
//window.onload = logoResize;
    ...
    function logoResize() {
        ...
        for (var i1 = 0; i1 < line.length; i1++) {
            line[i1] = {
                p1: [logoSize.offsetX + realityEditor.gui.ar.utilities.map(line[i1].p1[0], 0, 1785, 0, logoSize.W),
                    logoSize.offsetY + realityEditor.gui.ar.utilities.map(line[i1].p1[1], 0, 1035, 0, 1035 / 1785 * logoSize.W)]

                , p2: [logoSize.offsetX + realityEditor.gui.ar.utilities.map(line[i1].p2[0], 0, 1785, 0, logoSize.W),
                    logoSize.offsetY + realityEditor.gui.ar.utilities.map(line[i1].p2[1], 0, 1035, 0, 1035 / 1785 * logoSize.W)]

                , l1: realityEditor.gui.ar.utilities.map(line[i1].l1, 0, 1785, 0, logoSize.W),
                l2: realityEditor.gui.ar.utilities.map(line[i1].l2, 0, 1785, 0, logoSize.W),
        ...
        for (i1 = 0; i1 < thePoint.length; i1++) {
            thePoint[i1] = {
                p1: [logoSize.offsetX + realityEditor.gui.ar.utilities.map(thePoint[i1].p1[0], 0, 1785, 0, logoSize.W),
                    logoSize.offsetY + realityEditor.gui.ar.utilities.map(thePoint[i1].p1[1], 0, 1035, 0, 1035 / 1785 * logoSize.W)]

                , l1: realityEditor.gui.ar.utilities.map(thePoint[i1].l1, 0, 1785, 0, logoSize.W)
            };
        }

        for (i1 = 0; i1 < text.length; i1++) {
            text[i1] = {
                p1: [logoSize.offsetX + realityEditor.gui.ar.utilities.map(text[i1].p1[0], 0, 1785, 0, logoSize.W),
                    logoSize.offsetY + realityEditor.gui.ar.utilities.map(text[i1].p1[1], 0, 1035, 0, 1035 / 1785 * logoSize.W)]

                , l1: realityEditor.gui.ar.utilities.map(text[i1].l1, 0, 1785, 0, logoSize.W)
            };
        }

        shadow = realityEditor.gui.ar.utilities.map(shadow, 0, 1785, 0, logoSize.W);
    }
...
//ctx.scale(0.5,0.5);
    ...
    function logo() {
        ...
        for (var i1 = 0; i1 < line.length; i1++) {
            realityEditor.gui.ar.lines.drawLine(ctx, line[i1].p1, line[i1].p2, line[i1].l1, line[i1].l2, line[i1], timeCorrection, 0, 2, 0.04);
        }
