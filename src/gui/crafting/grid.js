...
realityEditor.gui.crafting.grid.addBlockLink = function(nodeA, nodeB, logicA, logicB, addToLogic, isServerOnlyLink) {
    if (nodeA && nodeB) {
        ...
        var linkKey = isServerOnlyLink ? this.edgeBlockLinkKey(blockLink) : "blockLink" + this.realityEditor.device.utilities.uuidTime();

        if (addToLogic) {
            if (!this.doesLinkAlreadyExist(blockLink)) {
                globalStates.currentLogic.links[linkKey] = blockLink;
            }
        }
        if (addToLogic || isServerOnlyLink) {
            this.uploadLinkIfNecessary(blockLink, linkKey);
        }
        return blockLink;
    }
    return null;
...
realityEditor.gui.crafting.grid.uploadLinkIfNecessary = function(blockLink, linkKey) {
    ...
    if (this.crafting.eventHelper.shouldUploadBlockLink(blockLink)) {
        ...
        var surroundingLinks = this.getLinksSurroundingEdgeLink(blockLink);
        if (surroundingLinks.length > 0) {
            surroundingLinks.forEach( function(link) {
                this.addBlockLink(link.nodeA, link.nodeB, link.logicA, link.logicB, false, true);
            });
        }
    }
};
...
realityEditor.gui.crafting.grid.getLinksSurroundingEdgeLink = function(edgeLink) {
    ...
    if (this.isEdgePlaceholderBlock(edgeLink.nodeA)) {
    ...
    } else if (this.isEdgePlaceholderBlock(edgeLink.nodeB)) {
...
realityEditor.gui.crafting.grid.addBlock = function(x,y,blockJSON,globalId,isEdgeBlock) {
    ...
    if (block.y === 0 || block.y === 3) {
        this.updateInOutLinks(globalId);
    }

    if (this.crafting.eventHelper.shouldUploadBlock(block)) {
        var keys = this.crafting.eventHelper.getServerObjectLogicKeys(globalStates.currentLogic);
        this.realityEditor.network.postNewBlock(keys.ip, keys.objectKey, keys.logicKey, block.globalId, block);
    }
    return block;
};
    ...
    for (var i = 0; i < addedBlock.blockSize; i++) {
        ...
        for (var key in globalStates.currentLogic.links) {
            ...
            if (link.nodeA === inOutName || link.nodeB === inOutName) {
                this.removeBlockLink(key);
            }
        }

        if (addedBlock.y === 0) {
            this.addBlockLink(inOutName, addedBlock.globalId, 0, i, true);
        } else {
            this.addBlockLink(addedBlock.globalId, inOutName, i, 0, true);
        }
...
realityEditor.gui.crafting.grid.removeBlockLink = function(linkKey) {
    if (this.crafting.eventHelper.shouldUploadBlockLink(globalStates.currentLogic.links[linkKey])) {
        ...
        this.deleteSurroundingBlockLinksFromServer(linkKey);
    }
...
realityEditor.gui.crafting.grid.removeBlock = function(logic, blockID) {
    ...
    if (this.crafting.eventHelper.shouldUploadBlock(logic.blocks[blockID])) {
        var keys = this.crafting.eventHelper.getServerObjectLogicKeys(logic);
        this.realityEditor.network.deleteBlockFromObject(keys.ip, keys.objectKey, keys.logicKey, blockID);
    }
...
realityEditor.gui.crafting.grid.removeLinksForBlock = function(logic, blockID) {
    for (var linkKey in logic.links) {
        ...
        if (link.nodeA === blockID || link.nodeB === blockID) {
            if (this.crafting.eventHelper.shouldUploadBlockLink(link)) {
                var keys = this.crafting.eventHelper.getServerObjectLogicKeys(logic);
                this.realityEditor.network.deleteBlockLinkFromObject(keys.ip, keys.objectKey, keys.logicKey, linkKey);
            } else {
                this.deleteSurroundingBlockLinksFromServer(linkKey);
...
realityEditor.gui.crafting.grid.deleteSurroundingBlockLinksFromServer = function(linkKey) {
    ...
    if (surroundingLinks.length > 0) {
        surroundingLinks.forEach( function(link) {
            this.realityEditor.network.deleteBlockLinkFromObject(keys.ip, keys.objectKey, keys.logicKey, this.edgeBlockLinkKey(link));
        });
    }
};
...
realityEditor.gui.crafting.grid.doesLinkAlreadyExist = function(blockLink) {
    ...
    for (var linkKey in globalStates.currentLogic.links) {
        ...
        if (this.areBlockLinksEqual(blockLink, thatBlockLink)) {
            return true;
        }
    }
    return false;
